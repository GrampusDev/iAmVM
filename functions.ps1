...
function AddAuditToFile {
    ...
    Get-Acl -Path $path -Audit | Format-List Path,AuditToString | Out-File -FilePath '.\logs\file_before.txt' -Width 200 -Append
    ...
    Get-Acl -Path $path -Audit | Format-List Path,AuditToString | Out-File -FilePath '.\logs\file_after.txt' -Width 200 -Append

    $fileObject=(get-item $path)
    if($fileObject){
        $parentFolder=$fileObject.Directory
        Get-Acl $parentFolder -Audit | Format-List Path,AuditToString | Out-File -FilePath '.\logs\file_before.txt' -Width 200 -Append
        ...
        Get-Acl $parentFolder -Audit | Format-List Path,AuditToString | Out-File -FilePath '.\logs\file_after.txt' -Width 200 -Append
        }
        else {
...
function DelAuditFromFile {
    ...
    $path=Resolve-Path($path)
    Get-Acl $path -Audit | Format-List Path,AuditToString | Out-File -FilePath '.\logs\file_before_delete.txt' -Width 200 -Append
    $File_ACL = Get-Acl $path
    $AccessRule = New-Object System.Security.AccessControl.FileSystemAuditRule("Everyone","Read","none","none","Success")
    $File_ACL.RemoveAuditRule($AccessRule)
    $File_ACL | Set-Acl $path
    Get-Acl $path -Audit | Format-List Path,AuditToString | Out-File -FilePath '.\logs\file_after_delete.txt' -Width 200 -Append
...
function AddAuditToRegKey {
    ...
    Get-Acl $key -Audit | Format-List Path,AuditToString | Out-File -FilePath '.\logs\reg_before.txt' -Width 200 -Append
    ...
    Get-Acl $key -Audit | Format-List Path,AuditToString | Out-File -FilePath '.\logs\reg_after.txt' -Width 200 -Append
}
...
function DelAuditFromRegKey {
    ...
    Get-Acl $key -Audit | Format-List Path,AuditToString | Out-File -FilePath '.\logs\reg_before_delete.txt' -Width 200 -Append
    $RegKey_ACL = Get-Acl $key
    ...
    $RegKey_ACL | Set-Acl $key
    Get-Acl $key -Audit | Format-List Path,AuditToString | Out-File -FilePath '.\logs\reg_after_delete.txt' -Width 200 -Append
}
