...
class MalwareClient(object):
    ...
    def is_vm_by_ip_id(self, ip_id):
        ...
        """
        if ('Windows' in self.client_os and
                abs(ip_id - self.last_ip_id) <= IP_ID_THRESHOLD):
            return False
        elif ('Linux' in self.client_os and
                abs(ip_id - self.last_ip_id) > IP_ID_THRESHOLD):
            return False
        else:
            return True
    ...
    def is_vm_by_ttl(self, ttl):
        ...
        :return: True if VM else False.
        """
        if self.client_os == 'Windows' and (128 - ttl) <= TTL_THRESHOLD:
            return False
        elif self.client_os == 'Linux' and (64 - ttl) <= TTL_THRESHOLD:
            return False
        else:
            return True
